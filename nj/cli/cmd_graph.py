...
def _run_show(db_path: str) -> None:
    ...
    if not person_nodes:
        console.print("[yellow]Graph is empty.[/yellow] Run [bold]nj graph build[/bold] first.")
        return

    person = person_nodes[0]
    tree = Tree(f"[bold cyan]{person.label}[/bold cyan] [dim](you)[/dim]")

    skill_neighbors = repo.get_neighbors(person.id, "HAS_SKILL")
    if skill_neighbors:
        skills_branch = tree.add(f"[green]Skills[/green] [dim]({len(skill_neighbors)})[/dim]")
        for n in sorted(skill_neighbors, key=lambda x: x["weight"], reverse=True)[:10]:
            strength = "●" * min(int(n["weight"] * 3), 5)
            skills_branch.add(f"[green]{n['label']}[/green] [dim]{strength}[/dim]")
        if len(skill_neighbors) > 10:
            skills_branch.add(f"[dim]... and {len(skill_neighbors) - 10} more[/dim]")

    company_neighbors = repo.get_neighbors(person.id, "WORKED_AT")
    if company_neighbors:
        co_branch = tree.add(f"[yellow]Companies[/yellow] [dim]({len(company_neighbors)})[/dim]")
        for n in company_neighbors:
            title = n.get("properties", {}).get("title", "")
            co_branch.add(f"[yellow]{n['label']}[/yellow] [dim]{title}[/dim]")

    project_neighbors = repo.get_neighbors(person.id, "BUILT")
    if project_neighbors:
        proj_branch = tree.add(f"[magenta]Projects[/magenta] [dim]({len(project_neighbors)})[/dim]")
        for n in project_neighbors:
            anchor = " [cyan][anchor][/cyan]" if n.get("properties", {}).get("anchor") else ""
    ...
    if roles_only:
        role_branch = tree.add(f"[red]Applied roles[/red] [dim]({len(roles_only)})[/dim]")
        for n in sorted(
            roles_only,
            ...
            role_branch.add(f"[red]{n['label']}[/red] [dim]score: {score}[/dim]")
...
def _run_path(from_label: str, to_label: str, db_path: str) -> None:
    ...
    for i, node in enumerate(path):
        ...
        console.print(f"  {arrow}[{color}]{node['label']}[/{color}] [dim]({node['type']})[/dim]")
...
def _run_skills(db_path: str) -> None:
    ...
    if not skills:
        console.print("[yellow]No skills in graph yet.[/yellow] Run [bold]nj graph build[/bold].")
        return
