...
def analyze_postmortem(
    ...
    interviews = [
        a for a in applications if a.outcome in (OutcomeType.INTERVIEW, OutcomeType.OFFER)
    ]
    rejections = [a for a in applications if a.outcome == OutcomeType.REJECTION]
    no_response = [
        a for a in applications if a.outcome == OutcomeType.NO_RESPONSE or a.outcome is None
    ]
    ...
    if report.total_applications > 0:
        report.interview_rate = round(len(interviews) / report.total_applications * 100, 1)
        report.rejection_rate = round(len(rejections) / report.total_applications * 100, 1)
        report.no_response_rate = round(len(no_response) / report.total_applications * 100, 1)

    all_scores = [scores[a.job_id].total_score for a in applications if a.job_id in scores]
    interview_scores = [scores[a.job_id].total_score for a in interviews if a.job_id in scores]
    rejection_scores = [scores[a.job_id].total_score for a in rejections if a.job_id in scores]

    report.avg_score_all = round(sum(all_scores) / len(all_scores), 1) if all_scores else 0.0
    report.avg_score_interviews = (
        round(sum(interview_scores) / len(interview_scores), 1) if interview_scores else 0.0
    )
    report.avg_score_rejections = (
        round(sum(rejection_scores) / len(rejection_scores), 1) if rejection_scores else 0.0
    )
    ...
    scored_apps = [(a, scores[a.job_id]) for a in applications if a.job_id in scores]
    ...
    report.role_type_analysis = _analyze_role_types(applications, scores, jobs, interviews)
    report.skill_gap_patterns = _analyze_skill_gaps(applications, scores, rejections)
...
def _detect_patterns(
    ...
    low_score_apps = [
        a for a in applications if a.job_id in scores and scores[a.job_id].total_score < 60
    ]
    if len(low_score_apps) > len(applications) * 0.3:
        ...
        patterns.append(
            ApplicationPattern(
                ...
                severity="high",
                evidence=[f"Score range: {min(low_vals)}–{max(low_vals)}"],
                recommendation=(
    ...
    if interviews and rejections:
        int_scores = [scores[a.job_id].total_score for a in interviews if a.job_id in scores]
        rej_scores = [scores[a.job_id].total_score for a in rejections if a.job_id in scores]
        if int_scores and rej_scores:
            diff = abs(sum(int_scores) / len(int_scores) - sum(rej_scores) / len(rej_scores))
            if diff < 5:
                patterns.append(
                    ApplicationPattern(
    ...
    visa_blocked = [
        a for a in applications if a.job_id in jobs and jobs[a.job_id].visa_label.value == "blocked"
    ]
    if visa_blocked:
        patterns.append(
            ApplicationPattern(
                pattern_type="visa_blocked_applications",
                description=(f"{len(visa_blocked)} applications sent to visa-blocked companies"),
                severity="high",
                evidence=[
                    "Companies: "
                    + ", ".join(
                        jobs[a.job_id].company for a in visa_blocked[:3] if a.job_id in jobs
                    )
                ],
                recommendation=(
                    "Enable skip_no_sponsorship in config. These applications will never convert."
                ),
            )
        )
        ...
        if avg_exp < 55:
            patterns.append(
                ApplicationPattern(
                    ...
                    description=(
                        f"Average experience_relevance score: {avg_exp:.0f}/100 — consistently low"
                    ),
                    severity="high",
                    evidence=[
...
def _analyze_role_types(applications, scores, jobs, interviews) -> dict:
    ...
    for app in applications:
        ...
        if app.job_id in scores:
            role_outcomes[role_type]["avg_score"].append(scores[app.job_id].total_score)
    ...
    for role, data in role_outcomes.items():
        ...
        result[role] = {
            ...
            "interview_rate": round(data["interviews"] / data["total"] * 100, 1)
            if data["total"] > 0
            else 0,
...
def _analyze_skill_gaps(applications, scores, rejections) -> list[dict]:
...
def _generate_recommendations(report: PostmortemReport) -> list[str]:
    ...
    if report.avg_score_all < 65:
        recs.append(
            f"Average score {report.avg_score_all} is low — raise threshold and be more selective"
        )

    if (
        report.avg_score_interviews > report.avg_score_rejections + 8
    ...
    if not recs:
        recs.append(
            "Not enough outcome data yet. Keep applying and run nj watch to track callbacks."
        )
